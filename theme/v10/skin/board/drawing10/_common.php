...
foreach ($set_values as $set_value) {
	list($key, $value) = explode('=', $set_value);
	$g5['set_board_grades'][$key] = $value.' ('.$key.')';
	$g5['set_board_grades_value'][$key] = $value;
	$g5['set_board_grades_radios'] .= '<label for="set_board_grades_'.$key.'" class="set_board_grades"><input type="radio" id="set_board_grades_'.$key.'" name="set_board_grades" value="'.$key.'">'.$value.'('.$key.')</label>';
	$g5['set_board_grades_checkboxs'] .= '<label for="set_board_grades_'.$key.'" class="set_board_grades"><input type="hidden" name="set_board_grades_'.$key.'" value=""><input type="checkbox" id="set_board_grades_'.$key.'">'.$value.'('.$key.')</label>';
	$g5['set_board_grades_buttons'] .= '<a href="javascript:" class="set_board_grades" cmm_status="'.$key.'">'.$value.'</a>';
	$g5['set_board_grades_options'] .= '<option value="'.trim($key).'">'.trim($value).' ('.$key.')</option>';
	$g5['set_board_grades_options_value'] .= '<option value="'.trim($key).'">'.trim($value).'</option>';
}
unset($set_values);unset($set_value);
