...
foreach ($set_values as $set_value) {
	list($key, $value) = explode('=', $set_value);
	$g5['set_board_period'][$key] = $value.' ('.$key.')';
	$g5['set_board_period_value'][$key] = $value;
	$g5['set_board_period_radios'] .= '<label for="set_board_period_'.$key.'" class="set_board_period"><input type="radio" id="set_board_period_'.$key.'" name="set_board_period" value="'.$key.'">'.$value.'('.$key.')</label>';
	$g5['set_board_period_checkboxs'] .= '<label for="set_board_period_'.$key.'" class="set_board_period"><input type="hidden" name="set_board_period_'.$key.'" value=""><input type="checkbox" id="set_board_period_'.$key.'">'.$value.'('.$key.')</label>';
	$g5['set_board_period_buttons'] .= '<a href="javascript:" class="set_board_period" cmm_status="'.$key.'">'.$value.'</a>';
	$g5['set_board_period_options'] .= '<option value="'.trim($key).'">'.trim($value).' ('.$key.')</option>';
	$g5['set_board_period_options_value'] .= '<option value="'.trim($key).'">'.trim($value).'</option>';
}
unset($set_values);unset($set_value);

// 발송시간
$set_values = explode(',', preg_replace("/\s+/", "", $board['bo_6']));
foreach ($set_values as $set_value) {
	list($key, $value) = explode('=', $set_value);
	$g5['set_board_time'][$key] = $value.' ('.$key.')';
	$g5['set_board_time_value'][$key] = $value;
	$g5['set_board_time_radios'] .= '<label for="set_board_time_'.$key.'" class="set_board_time"><input type="radio" id="set_board_time_'.$key.'" name="set_board_time" value="'.$key.'">'.$value.'('.$key.')</label>';
	$g5['set_board_time_checkboxs'] .= '<label for="set_board_time_'.$key.'" class="set_board_time"><input type="hidden" name="set_board_time_'.$key.'" value=""><input type="checkbox" id="set_board_time_'.$key.'">'.$value.'('.$key.')</label>';
	$g5['set_board_time_buttons'] .= '<a href="javascript:" class="set_board_time" cmm_status="'.$key.'">'.$value.'</a>';
	$g5['set_board_time_options'] .= '<option value="'.trim($key).'">'.trim($value).' ('.$key.')</option>';
	$g5['set_board_time_options_value'] .= '<option value="'.trim($key).'">'.trim($value).'</option>';
}
unset($set_values);unset($set_value);
