...
// URL: G5_URL/theme/v10/mobile/skin/board/schedule11m/ajax.calendar.php
...
//                    echo $row[$key].': '.$val.' ---- <br>';
                    ...
                    $day_content[$row[$key]] .= '<div class="certify_item certify_'.$row['crt_status'].'" crt_idx="'.$row['crt_idx'].'">'
                                                    ...
                                                    .'<span class="certify_text">'.$val.'</span>'
...
//    echo $sql1;
    ...
    for ($i=0; $row=sql_fetch_array($rs1); $i++) {
        ...
        $day_content[$row['crj_date']] .= '<div class="certify_item certify_'.$row['crj_status'].'" crj_idx="'.$row['crj_idx'].'">'
                                        ...
                                        .'<span class="certify_code_name">'.$g5['set_crt_codes_value'][$row['crt_code']].'</span>'
                                        .'<span class="certify_judge_name">'.$g5['set_crj_type_value'][$row['crj_type']].'</span>'
                                        .'<span class="certify_com_name"><a href="'.$row['href'].'">'.$row['com_name'].'</a></span>'
    ...
    // 날짜별 상태별 통계 { -------
    $sql  = "SELECT SUBSTRING(wr_2,1,10) AS wr_ymd, wr_9, COUNT(wr_id) AS wr_sum
...
//        $row['wr_ampm'] = date("A h:i ", strtotime(preg_replace("/-/","/",$row['wr_3'])) );
//        $row['href'] = get_pretty_url($bo_table, $row['wr_id']);
//
//        
//        // 일정표현
//        $day_content[substr($row['wr_2'],0,10)] .= '<div class="schedule_item" wr_id="'.$row['wr_id'].'" class="schedule_'.$row['wr_9'].'">'
...
//            // 공휴일 내용이 있으면 점 하나만 찍어줌
//            echo ($row[$i][$j]['holiday_name']) ? '<div class="day_holiday_dot"><i class="fa fa-circle"></i></div>' 
//                                                    : '' ;
            echo '</div>';
        }
        ...
        // 공휴일 내용
        echo ($row[$i][$j]['holiday_name']) ? '<span class="day_holiday_dot"><i class="fa fa-circle"></i></span>'
                                            : '' ;
        
        // 만료일 내용
        echo ($day_expire[$row['day'.$j]]) ? '<span class="day_expire_dot"><i class="fa fa-circle"></i></span>'
                                            : '' ;

        // 심사일정 내용
        echo ($day_judge[$row['day'.$j]]) ? '<span class="day_judge_dot"><i class="fa fa-circle"></i></span>'
                                            : '' ;
    
        // 일정 있으면 점
        echo ($day_total[$row['day'.$j]]) ? '<span class="day_content_dot"><i class="fa fa-circle"></i></span>'
                                            : '' ;
