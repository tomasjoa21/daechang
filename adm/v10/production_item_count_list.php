...
// 검색 조건
$st_date = $st_date ?: statics_date(G5_TIME_YMDHIS);
$en_date = $en_date ?: statics_date(G5_TIME_YMDHIS);
...
if ($st_date)	// 시작일 있는 경우
    $where[] .= " pic_date >= '{$st_date}' ";
if ($en_date)	// 종료일 있는 경우
    $where[] .= " pic_date <= '{$en_date}' ";
...
if ($ser_cst_idx_customer) {
    $sql1 = " SELECT bom_idx FROM {$g5['bom_table']} WHERE cst_idx_customer = '".trim($ser_cst_idx_customer)."' ";
    // echo $sql1.'<br>';
    $rs1 = sql_query($sql1,1);
    for($k=0;$row1=sql_fetch_array($rs1);$k++) {
        $ser_bom_idxs[] = $row1['bom_idx'];
    }
    // print_r2(array_unique($ser_bom_idxs));
    if($ser_bom_idxs[0])
        $where[] = " bom_idx IN (".implode(",",array_unique($ser_bom_idxs)).") ";
    else
        $where[] = " (0) ";
}
// 공급사
if ($ser_cst_idx_provider) {
    $sql1 = " SELECT bom_idx FROM {$g5['bom_table']} WHERE cst_idx_provider = '".trim($ser_cst_idx_provider)."' ";
    // echo $sql1.'<br>';
    $rs1 = sql_query($sql1,1);
    for($k=0;$row1=sql_fetch_array($rs1);$k++) {
        $ser_bom_idxs[] = $row1['bom_idx'];
...
// 작업상태
if (isset($ser_pic_ing)&&$ser_pic_ing) {
    $where[] = " pic_ing = '".$ser_pic_ing."' ";
}
...
<div class="local_desc01 local_desc" style="display:no ne;">
    <p>상세한 검색을 위해서는 [상세] 버튼을 클릭하시면 됩니다.</p>
</div>

<form id="fsearch" name="fsearch" class="local_sch01 local_sch" method="get" style="width:100%;">
<label for="sfl" class="sound_only">검색대상</label>
통계일: 
<input type="text" name="st_date" value="<?=$st_date ?>" id="st_date" class="frm_input" style="width:90px;"> ~
<input type="text" name="en_date" value="<?=$en_date ?>" id="en_date" class="frm_input" style="width:90px;">
		...
		<table>
			...
			<tbody>
				...
				<tr>
                    ...
                    <th>작업상태</th>
					<td>
						<input type="radio" name="ser_pic_ing" id="ser_pic_ing_all" value="" checked=""><label for="ser_pic_ing_all">관계없음</label>
						<?php
                        // print_r2($g5['set_pri_ing_value']);
                        if(is_array($g5['set_pri_ing_value'])) {
                            foreach ($g5['set_pri_ing_value'] as $k1=>$v1) {
                                echo '<input type="radio" name="ser_pic_ing" id="ser_pic_ing_'.$k1.'" value="'.$k1.'">
                                      <label for="ser_pic_ing_'.$k1.'">'.$v1.'</label>'.PHP_EOL;
                            }
                        }
						?>
						<script>$('#ser_pic_ing_<?=$ser_pic_ing?>').attr('checked','checked');</script>
                    </td>
				</tr>
			</tbody>
		</table>
...
// ser_st_date chage
$(document).on('focusin', 'input[name=ser_st_date]', function(){
    // console.log("Saving value: " + $(this).val());
    $(this).data('val', $(this).val());
}).on('change','input[name=ser_st_date]', function(){
    var prev = $(this).data('val');
    var current = $(this).val();
    // console.log("Prev value: " + prev);
    // console.log("New value: " + current);
    if(prev=='') {
        $('input[name=ser_st_time]').val('00:00:00');
    }
});
// ser_en_date chage
$(document).on('focusin', 'input[name=ser_en_date]', function(){
    $(this).data('val', $(this).val());
}).on('change','input[name=ser_en_date]', function(){
    var prev = $(this).data('val');
    if(prev=='') {
        $('input[name=ser_en_time]').val('23:59:59');
    }
});
