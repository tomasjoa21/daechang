...
$sql_common = " FROM g5_1_engrave_qrcode ";
...
if(sizeof($where)<=1) {
    $sql = " SELECT row_estimate AS cnt FROM hypertable_approximate_row_count('g5_1_engrave_qrcode') ";
}
else {
...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array_pg($result); $i++) {
        ...
        $s_mod_a = '<a href="./'.$fname.'_form.php?'.$qstr.'&w=u&eqr_idx='.$row['eqr_idx'].'">';
        $s_mod = '<a href="./'.$fname.'_form.php?'.$qstr.'&w=u&eqr_idx='.$row['eqr_idx'].'" class="btn btn_03">수정</a>';
        $s_copy = '<a href="./'.$fname.'_form.php?'.$qstr.'&w=c&eqr_idx='.$row['eqr_idx'].'" class="btn btn_03">복제</a>';

        echo '
			<tr tr_id="'.$i.'" style="background-color:'.$row['tr_bgcolor'].';color:'.$row['tr_color'].'">
				<td>'.$s_mod_a.$row['eqr_idx'].'</a></td>
				<td>'.$row['work_date'].'</td>
