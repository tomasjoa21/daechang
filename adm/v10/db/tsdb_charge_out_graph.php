...
$sql = " SELECT * FROM g5_1_charge_out ORDER BY cho_idx DESC LIMIT 1 ";
$one = sql_fetch($sql,1);
// print_r3($one);
...
// item_type
$item_type = ($item_type) ? $item_type : 'weight_out';
// query string
...
<form id="fsearch" name="fsearch" class="local_sch01 local_sch" method="get">
    <select name="item_type">
        <option value="weight_out">출탕량</option>
        <option value="temp_out">출탕온도</option>
        <option value="temp_gbf">GBF후온도</option>
    </select>
    <script>
...
<script>
// Detail graph
Highcharts.getJSON(g5_url+'/device/rdb/charge_out.php?<?=$qs?>', function(data) {
