...
$sql_common = " FROM g5_1_melting_temp ";
...
if(sizeof($where)<=1) {
    $sql = " SELECT row_estimate AS cnt FROM hypertable_approximate_row_count('g5_1_melting_temp') ";
}
else {
...
<div class="tbl_head01 tbl_wrap">
	...
	<tr>
		...
		<th scope="col">평균온도</th>
		<th scope="col">최소온도</th>
		<th scope="col">최대온도</th>
		<th scope="col">최소알람기준</th>
		<th scope="col">최대알람기준</th>
    ...
    for ($i=0; $row=sql_fetch_array_pg($result); $i++) {
        ...
        $s_mod_a = '<a href="./'.$fname.'_form.php?'.$qstr.'&w=u&mlt_idx='.$row['mlt_idx'].'">';
        $s_mod = '<a href="./'.$fname.'_form.php?'.$qstr.'&w=u&mlt_idx='.$row['mlt_idx'].'" class="btn btn_03">수정</a>';
        $s_copy = '<a href="./'.$fname.'_form.php?'.$qstr.'&w=c&mlt_idx='.$row['mlt_idx'].'" class="btn btn_03">복제</a>';

        echo '
			<tr tr_id="'.$i.'" style="background-color:'.$row['tr_bgcolor'].';color:'.$row['tr_color'].'">
				<td>'.$s_mod_a.$row['mlt_idx'].'</a></td>
				<td>'.$row['work_date'].'</td>
				<td>'.$g5['set_work_shift'][$row['work_shift']].'('.$row['work_shift'].')</td>
