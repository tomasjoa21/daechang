...
$sql_common = " FROM g5_1_charge_out ";
...
if(sizeof($where)<=1) {
    $sql = " SELECT row_estimate AS cnt FROM hypertable_approximate_row_count('g5_1_charge_out') ";
}
else {
...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array_pg($result); $i++) {
        ...
        $s_mod_a = '<a href="./'.$fname.'_form.php?'.$qstr.'&w=u&cho_idx='.$row['cho_idx'].'">';
        $s_mod = '<a href="./'.$fname.'_form.php?'.$qstr.'&w=u&cho_idx='.$row['cho_idx'].'" class="btn btn_03">수정</a>';
        $s_copy = '<a href="./'.$fname.'_form.php?'.$qstr.'&w=c&cho_idx='.$row['cho_idx'].'" class="btn btn_03">복제</a>';

        echo '
			<tr tr_id="'.$i.'" style="background-color:'.$row['tr_bgcolor'].';color:'.$row['tr_color'].'">
				<td>'.$s_mod_a.$row['cho_idx'].'</a></td>
				<td>'.$row['work_date'].'</td>
