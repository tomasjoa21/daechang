...
foreach($_POST['chk'] as $moi_idx_v){
    ...
    for($i=0;$i<$input_cnt[$moi_idx_v];$i++){
        $mtr_sql .= ($i==0) ? '':',';
        $mtr_sql .= "('{$_SESSION['ss_com_idx']}','{$cst_idx_provider[$moi_idx_v]}','{$cst_idx_customer[$moi_idx_v]}','{$bom_idx[$moi_idx_v]}','{$moi_idx_v}','{$bom_name[$moi_idx_v]}','{$bom_part_no[$moi_idx_v]}','{$bom_price[$moi_idx_v]}','1','".G5_TIME_YMD."','material','ok','".G5_TIME_YMDHIS."','".G5_TIME_YMDHIS."','".G5_TIME_YMDHIS."')";
    }
    // echo $mtr_sql;
    sql_query($mtr_sql,1);
}
// exit;
goto_url('./material_order_input_list.php?'.$qstr);