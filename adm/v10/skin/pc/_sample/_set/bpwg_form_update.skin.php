...
//echo $bwgs_device."<br>";
//echo $bwgs_skin."<br>";
$link_arr = array('link1','link2','link3');
foreach($link_arr as $lnk){	
	if(isset($bwo[$lnk])){
		$g5_dns = preg_replace('/^http(s|)\:\/\//i','',G5_URL); //$g5_dns = preg_replace('/^http(s|)\:\/\//i','',G5_URL); //http(s)://이후의 url를 담는 배열
		$bwo[$lnk] = isset($bwo[$lnk]) ? trim(strip_tags($bwo[$lnk])) : '';
		if(preg_match("/^(?:(?:[a-z]+):\/\/)?((?:[a-z\d\-]{2,}\.)+[a-z]{2,})(?::\d{1,5})?(?:\/[^\?]*)?(?:\?.+)?[\#]{0,1}$/i",$bwo[$lnk])){
			if(preg_match('/'.$g5_dns.'/',$bwo[$lnk])){
				$bwo[$lnk] = preg_replace('/^(http(s|)\:\/\/|)(www\.|)?/i','',$bwo[$lnk]);
				$bwo[$lnk] = str_replace($g5_dns,'',$bwo[$lnk]);
			}
		}
	}
}
