...
$sql = " 
	...
	WHERE  
		(me_use = '1' AND LENGTH(me_code) = 2)
		OR
		(me_use = '1' AND LENGTH(me_code) = 4 AND (SELECT me_use FROM {$g5['menu_table']} WHERE me_code = SUBSTRING(me.me_code,1,2)) = '1')
		OR
		(me_use = '1' AND LENGTH(me_code) = 6 AND (SELECT me_use FROM {$g5['menu_table']} WHERE me_code = SUBSTRING(me.me_code,1,2)) = '1' AND (SELECT me_use FROM {$g5['menu_table']} WHERE me_code = SUBSTRING(me.me_code,1,4)) = '1')
...
//if(false){
	...
	echo '<div class="bwg_empty" style="background:#f1f1f1;text-align:center;padding:100px 0;border:1px solid #ddd;"><a href="'.G5_BPWIDGET_ADMIN_URL.'/bpwidget_form.php?bwgs_idx='.$bwgs_idx.'&w=u" target="_blank">['.strtoupper($bwg_arr['config']['bwgs_name']).']</a>의 내용이 존재하지 않습니다.</div>'.PHP_EOL;
}
?>
