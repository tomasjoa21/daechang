	...
	<div id="<?=$sid?>" class="dv_slk">
		<?php for($i=0;$i<count($conarr);$i++){
			...
			if($conarr[$i]['bwgc_ytb_url']){
				...
				$sld_prop = 'ytb_url="'.$conarr[$i]['bwgc_ytb_url'].'" data-property="{videoURL:\''.$conarr[$i]['bwgc_ytb_url'].'\',vol:\'90\',stopMovieBlur:false,containment:\'#'.($sid.$i).'\',showControls:false,startAt:1,anchor:\'top, center\',mute:true,autoPlay:true,opacity:1,quality:\'highres\',optimizeDisplay:true}"';
			}else{
				$sld_class = 'sld_img';
...
<?=$bid?>_slk.css({'width':<?=$bid?>_wd,'height':<?=$bid?>_ht});
