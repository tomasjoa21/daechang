...
$thumb_width = 600;
$thumb_height = 400;
$thum_ratio = $thumb_width / $thumb_height;
...
?>
<style>
#<?=$bid?> .lt_box .lt_con{height:<?=$lst_height?>px;}
...
#<?=$bid?> .lt_box .lt_con .lt_list .lt_list_dv .lt_list_lst .lt_sld_img .lt_sld_panel{display:no ne;position:absolute;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.3);padding:10px;}
...
<div class="lt_list">
	...
	<script>
	var <?=$bid?>_lt_con = $('#<?=$bid?> .lt_box .lt_con');
	var <?=$bid?>_lt_img = $('#<?=$bid?> .lt_box .lt_con .lt_list .lt_list_dv .lt_list_lst .lt_sld_img');
	var <?=$bid?>_ratio = <?=$thum_ratio?>;
	var <?=$bid?>_wd = $('body').width() - <?=($lst_horizontal_interval*2)?>;
	...
	<?php } ?>
	<?=$bid?>_lt_con.css({'width':<?=$bid?>_wd,'height':<?=$bid?>_ht});
	<?=$bid?>_lt_img.css({'width':<?=$bid?>_wd,'height':<?=$bid?>_ht});
	$(window).resize(function(){
		<?=$bid?>_wd = $('body').width() - <?=($lst_horizontal_interval*2)?>;
