...
label[for^=mtr_defect_type] {margin-right:5px;}
</style>
	...
	<table>
        ...
        <tr>
            ...
            <td>
                ...
                <input type="hidden" name="mtr_part_no" value="<?=${$pre}['mtr_part_no']?>">
                <input type="text" name="mtr_name" value="<?=${$pre}['mtr_name']?>" class="frm_input required readonly" required readonly>
        ...
        <tr>
            ...
            <th scope="row">발주선택</th>
            <td>
                <input type="hidden" name="mto_idx" value="<?=${$pre}['mto_idx']?>" class="frm_input">
                <input type="text" name="moi_idx" value="<?=${$pre}['moi_idx']?>" class="frm_input" style="width:70px;" readonly>
                <a href="./material_order_item_select.php?file_name=<?=$g5['file_name']?>" class="btn btn_02 btn_material_order_item">찾기</a>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" name="shf_name" value="<?=$shf['shf_name']?>" class="frm_input readonly" style="width:90px" readonly>
        ...
        <tr>
            ...
            <td>
                <input type="text" name="mtr_auth_dt" value="<?=${$pre}['mtr_auth_dt']?>" class="frm_input" style="width:150px;" placeholder="승인일시">
                <input type="checkbox" value="<?php echo date("Y-m-d H:i:s"); ?>" id="auth_set_today" onclick="if
                    (this.form.mtr_auth_dt.value==this.form.mtr_auth_dt.defaultValue) { this.form.mtr_auth_dt.value=this.value; } else {
                    this.form.mtr_auth_dt.value=this.form.mtr_auth_dt.defaultValue; }">
                <label for="auth_set_today">오늘날짜로 지정</label>
            </td>
        </tr>
        <tr>
            <?php
            ...
            unset($ar);
            ?>
            <th scope="row">불량여부</th>
            <td>
                <label><input type="radio" name="<?=$pre?>_defect" value="0" <?=(!${$pre}[$pre.'_defect_type'])?'checked':''?>> 양품</label>
                &nbsp;
                <label><input type="radio" name="<?=$pre?>_defect" value="1" <?=(${$pre}[$pre.'_defect_type'])?'checked':''?>> 불량</label>
            </td>
        </tr>
        <tr class="tr_mtr_defect_type" style="display:<?=(${$pre}[$pre.'_defect_type'])?'':'none'?>;">
            <th scope="row">불량타입</th>
            <td colspan="3">
                <?php
                if(is_array($g5['set_mtr_defect_type_value'])) {
                    foreach ($g5['set_mtr_defect_type_value'] as $k1=>$v1) {
                        if(in_array($k1,array('trash'))) {continue;}
                        echo '<input type="radio" name="mtr_defect_type" id="mtr_defect_type_'.$k1.'" value="'.$k1.'">
                                <label for="mtr_defect_type_'.$k1.'">'.$v1.'</label>'.PHP_EOL;
                    }
                }
                ?>
                <script>$('#mtr_defect_type_<?=${$pre}['mtr_defect_type']?>').attr('checked','checked');</script>
                <script>
                    $(document).on('change','input[name=mtr_defect]',function(e){
                        // 불량이 체크된 경우
                        if( $(this).val() == 1 ) {
                            $('.tr_mtr_defect_type').show();
                        }
                        else {
                            $('.tr_mtr_defect_type').hide();
                            $('input[name=mtr_defect_type]').prop('checked',false);
                            $('input[name=mtr_defect_text]').val('');
                        }
                    });
                </script>
                <p style="margin-top:5px;">
                    <input type="text" name="mtr_defect_text" value="<?=${$pre}['mtr_defect_text']?>" class="frm_input" placeholder="불량이유">
                </p>
            </td>
        </tr>
        <?php
...
$(function(e) {
    ...
    // 발주 찾기
    $(document).on('click','.btn_material_order_item',function(e){
        e.preventDefault();
        var href = $(this).attr('href');
        winMaterialOrderItem = window.open(href, "winMaterialOrderItem", "left=100,top=100,width=520,height=600,scrollbars=1");
        winMaterialOrderItem.focus();
        return false;
