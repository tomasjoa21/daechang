...
$_REQUEST['mst_name'] = trim($_REQUEST['mst_name']);

// 중복체크
$sql = "SELECT COUNT(*) AS cnt
		FROM {$g5['mms_status_table']}
		WHERE mst_name = '".$_REQUEST['mst_name']."'
			AND mms_idx = '".$_REQUEST['mms_idx']."'
			AND mst_type = 'offwork'
			AND mst_status NOT IN('delete','del','trash','cancel')
";
$one = sql_fetch($sql);
if($one['cnt']){
	$response->err_code='duplicate';
	$response->msg = "이미 등록된 유형입니다.";
}
else {
