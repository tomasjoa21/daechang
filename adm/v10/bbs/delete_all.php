...
if(!$is_admin)
    alert('접근 권한이 없습니다.', G5_USER_ADMIN_URL);
...
for ($i=$chk_count-1; $i>=0; $i--)
{
    ...
    // 해당 메타이블의 정보 삭제
    sql_query(" delete from {$g5['meta_table']} where mta_db_table = 'board/{$bo_table}' and mta_db_id = '{$write['wr_id']}' ");
...
goto_url(short_url_clean(G5_USER_ADMIN_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;page='.$page.'&amp;'.$qstr));
?>
