...
for($i=0;$i<=sizeof($allData[0]);$i++) {
        ...
        && is_numeric($arr['bom_usage']) )
    {
        ...
        if($excel_type=='01') {
            ...
            if($bom_level == 2) {
                $num --;    // $num is reduced by the group level 2.
            }
            if($bom_level <= 3) {
                $reply = '';    // reset prev $reply.
            }
            ...
            if($bom_level > 2) {
                ...
                $reply_pre = ($bom_level>3) ? substr($reply,0,($bom_level-3)) : '';   // prior 2digit letters for level 5, prior 3digit letters for level 6
                $reply_len = $bom_level - 2;
                $sql_reply = ($reply) ? " AND bit_reply like '{$reply}%' " : "";
                $sql = "SELECT MAX(SUBSTRING(bit_reply, $reply_len, 1)) AS reply FROM {$g5['bom_item_table']}
                        WHERE bit_num = '{$num}' AND bom_idx = '".$bom_idx."'
                            AND SUBSTRING(bit_reply, {$reply_len}, 1) <> '' {$sql_reply}
                ";
                // if($demo) {print_r3($sql);}
                $bit = sql_fetch($sql,1);
                $reply_char = (!$bit['reply']) ? '10' : $reply_char = chr(ord($bit['reply']) + 1);
