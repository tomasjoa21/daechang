...
$_REQUEST['com_idx'] = $_SESSION['ss_com_idx'];
...
for($i=0; $row=sql_fetch_array($result); $i++) {
    ...
    // print_r2($row);
    $ar['cod_idx'] = $row['cod_idx'];
    $ar['cod_code'] = $row['cod_code'];
    $ar['mms_idx'] = $row['mms_idx'];
    $ar['mms_name'] = addslashes($row['mms']['mms_name']);
    $ar['com_idx'] = $row['com_idx'];
    $ar['cod_group'] = $row['cod_group'];
    $ar['cod_type'] = $row['cod_type'];
    $ar['cod_name'] = $row['cod_name'];
    $ar['cod_interval'] = $row['cod_interval'];
    $ar['cod_count'] = $row['cod_count'];
    $ar['cod_count_limit'] = $row['cod_count_limit'];
    $ar['cod_send_type'] = $row['cod_send_type'];
    $list[$row['cod_plc_ip']][$row['cod_plc_port']][$row['cod_plc_no']][$row['cod_plc_bit']] = $ar;
    unset($ar);
}
// print_r2($list);
// print_r2($list_idx2);
...
$cache_content .= "\$g5['socket_alarm']=".var_export($list, true).";\n";
$cache_content .= "?>";
