...
$table1 = 'MES_CAST_SHOT';

$table2 = 'g5_1_cast_shot';
...
// 한달씩
    ...
    $search1 = " WHERE WORK_DATE >= '".$ym."-01 00:00:00' AND WORK_DATE <= '".$ym."-31 23:59:59' AND END_TIME != '' ";     
}
// 하루씩
else if($ymd) {
    // $search1 = " WHERE EVENT_TIME LIKE '".$ymd."%' ";
    $search1 = " WHERE WORK_DATE >= '".$ymd." 00:00:00' AND WORK_DATE <= '".$ymd." 23:59:59' AND END_TIME != '' ";
    // $search1 = " WHERE CAMP_NO IN ('C0175987','C0175987') ";    // 특정레코드
}
else {
    // 데이터의 마지막 일시 ------
    $sql = " SELECT start_time FROM {$table2} ORDER BY csh_idx DESC LIMIT 1 ";
    $dat = sql_fetch($sql,1);
...
for ($i=0; $row=$result->fetch(PDO::FETCH_ASSOC); $i++) {
    ...
    // Record update
    $sql3 = "   SELECT csh_idx FROM {$table2}
                WHERE shot_id = '".$arr['shot_id']."'
    ";
    // echo $sql3.'<br>';
    ...
    echo "<script> document.all.cont.innerHTML += '".$cnt.". ".$arr['work_date']." (".$arr['shot_no'].":".$arr['start_time']."~".$arr['end_time'].") ".$arr['result']." 완료<br>'; </script>\n";
