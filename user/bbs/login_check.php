...
if ( is_use_email_certify() && !preg_match("/[1-9]/", $mb['mb_email_certify'])) {
    ...
    confirm("{$mb['mb_email']} 메일로 메일인증을 받으셔야 로그인 가능합니다. 다른 메일주소로 변경하여 인증하시려면 취소를 클릭하시기 바랍니다.", G5_USER_URL, G5_USER_BBS_URL.'/register_email.php?mb_id='.$mb_id.'&ckey='.$ckey);
}
...
if ($url) {
    // url 체크
    check_url_host($url, '', G5_USER_URL, true);
...
if( is_admin($mb['mb_id']) && is_dir(G5_DATA_PATH.'/tmp/') ){
    ...
    if($tmp_data_check){
        if(! @fwrite($tmp_data_check, G5_USER_URL)){
